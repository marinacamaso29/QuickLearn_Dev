  ...
  <div class="auth-page">
    ...
    <div class="card">
      ...
      <form v-if="step === 1" @submit="onRegister" class="form">
        ...
        <button class="primary" type="submit" :disabled="isLoading || !acceptedPrivacy || !isPasswordValid() || password !== confirmPassword">
...
.pp-icon { color: #667eea; }
.password-requirements { margin-top:8px; padding:12px; background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; }
.requirement { display:flex; align-items:center; gap:8px; margin:4px 0; font-size:13px; color:#64748b; }
.requirement.met { color:#059669; }
.check { font-weight:bold; font-size:14px; }
.password-mismatch { margin-top:4px; color:#dc2626; font-size:13px; }
...
.resend-btn { background:none; border:1px solid #d1d5db; color:#5562ea; padding:8px 16px; border-radius:8px; cursor:pointer; font-size:14px; transition:all 0.2s; }
.resend-btn:hover:not(:disabled) { border-color:#5562ea; background:#f8fafc; }
.resend-btn:disabled { opacity:0.5; cursor:not-allowed; color:#9ca3af; border-color:#e5e7eb; }
.otp-container { margin:16px 0; }
...
.otp-input { width:48px; height:48px; text-align:center; font-size:18px; font-weight:600; border:2px solid #d1d5db; border-radius:8px; background:#fff; transition:all 0.2s; }
.otp-input:focus { outline:none; border-color:#7b86f2; box-shadow:0 0 0 3px rgba(123,134,242,0.2); }
